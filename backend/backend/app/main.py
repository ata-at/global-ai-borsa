...
# TÜM BULUT ENGELLERİNİ GENEL VERİ KANALIYLA AŞAN SINIF
    ...
    async def time_series(self, symbol: str):
        try:
            ...
            async with httpx.AsyncClient(timeout=15.0, follow_redirects=True) as client:
                ...
                if is_crypto:
                    # Kriptolar için asla engellenmeyen küresel veri kanalı (Coingecko)
                    ...
                    for i in range(len(prices)):
                        ...
                        # Mum verisini simüle ediyoruz (Arayüz çizimi için)
                        formatted_values.append({
                            "datetime": str(tarih_formatli),
                            "open": prc * 0.995, "high": prc * 1.01,
                            "low": prc * 0.99, "close": prc,
                            "volume": vol
                        })
                else:
