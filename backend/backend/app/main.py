...
LAST = {}
PROCESSED_NEWS = set()  # Aynı haberleri tekrar analiz etmemek için hafıza
task = None
news_task = None

# HEM HİSSE HEM KRİPTO VERİLERİNİ KESİNTİSİZ ÇEKEN SINIF
class GlobalFinancialData:
    async def time_series(self, symbol: str):
        try:
            ...
            # DÜZELTME: Engellenmeyen Doğru Yahoo Finance API Kanalı
...
async def ask_ai_for_news_impact(title: str, content: str) -> dict:
    ...
    if not ai_api_key:
        # Yapay zeka API anahtarı girilmediyse çalışacak akıllı kelime filtresi
...
async def fetch_news_feeds():
    # DÜZELTME: Doğru formatta çalışan büyük haber akışları
    urls = [
    ...
    # TWITTER VE TELEGRAM KÖPRÜLERİ:
    # Belirttiğiniz devlet adamları, iş adamları ve haber kanallarını takip etmek için RSSHub hatları
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        for url in urls:
            try:
                ...
                for item in root.findall(".//item")[:3]: # Her kaynaktan son 3 anlık haberi al
                    ...
                    # Yapay zekaya haberi/tweeti yorumlatıyoruz
...
async def news_and_social_ai_loop():
    while True:
        ...
        await asyncio.sleep(120) # 2 dakikada bir 7/24 tarar
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["=*", "https://github.io"], # GitHub Pages arayüzüne izin verildi
