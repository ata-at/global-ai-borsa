...
class GlobalFinancialData:
    async def time_series(self, symbol: str):
        try:
            ...
            # Engellenmeyen Doğru Yahoo Finance API Kanalı
            ...
            result = chart_result[0] if isinstance(chart_result, list) else chart_result
            ...
            for i, ts in enumerate(timestamps):
                ...
                # DÜZELTME: Arayüzün desen (pattern) hatası vermemesi için düz metin tarih formatı
                tarih_metni = str(pd.to_datetime(ts, unit='s')).split(".")[0]
                
                formatted_values.append({
                    "datetime": tarih_metni,
...
async def ask_ai_for_news_impact(title: str, content: str) -> dict:
    ...
    try:
        ...
        prompt = (
            ...
            f'{{"direction": "YUKARI" veya "AŞAĞI" var "NÖTR", "summary": "Haberin kısa tek cümlelik özeti"}}'
        )
        payload = {
...
async def fetch_news_feeds():
    ...
    # Twitter ve Telegram Köprüleri (RSSHub üzerinden)
