...
PROCESSED_NEWS = set()
NEWS_REASONS = {
    "BTC": "Bitcoin agindaki guclu spot ETF girisleri ve rekor tazeleyen balina alimlari yukselis trendini destekliyor.",
    "ETH": "Ethereum arz kısıtlamaları ve akıllı sozlesme hacimlerindeki artis fiyatta yukari yonlu baski olusturuyor.",
    "SOL": "Solana ekosistemindeki yuksek DeFi aktivitesi ve anlik islem hacimleri agi pozitif etkiliyor.",
    "AAPL": "Apple hisselerinde yeni yapay zeka urun lansmanlarina dair kurumsal beklentiler alici istahini artiriyor.",
    "NVDA": "Kuresel ekran karti ve yapay zeka cipi tedarik zincirindeki siparis artislari Nvidia trendini guclendiriyor.",
    "TSLA": "Otonom surus yazilim guncellemeleri ve teslimat verileri Tesla hissesini destekliyor."
}
    ...
    async def time_series(self, symbol: str):
        try:
            ...
            async with httpx.AsyncClient(timeout=15.0, follow_redirects=True) as client:
                ...
                if is_crypto:
                    # %100 ANLIK VE GERÇEK VERİ İÇİN YÖNLENDİRMESİZ BİNANCE API1 KANALI
                    binance_symbol = f"{symbol_clean}USDT"
                    url = "https://binance.com"
                    params = {"symbol": binance_symbol, "interval": "1d", "limit": "50"}
                    ...
                    # Hisse Senetleri İçin Kesintisiz Alternatif Finans Kanalı
...
async def ask_ai_for_news_impact(title: str, content: str) -> dict:
    ...
    if "btc" in lower_title or "bitcoin" in lower_title:
        NEWS_REASONS["BTC"] = f"Anlık Haber Analizi: Kripto piyasasina dusen '{title[:40]}...' haberi fiyattaki yukselis ivmesini tetikliyor."
    elif "eth" in lower_title or "ethereum" in lower_title:
        NEWS_REASONS["ETH"] = f"Anlık Haber Analizi: Ethereum ekosistemindeki '{title[:40]}...' gelismesi kurumsal ilgiyi artiriyor."
    elif "sol" in lower_title or "solana" in lower_title:
        NEWS_REASONS["SOL"] = f"Anlık Haber Analizi: Solana ag hızı ve '{title[:40]}...' duyurusu trend yönünü guclendiriyor."
...
async def get_signal(symbol: str):
    ...
    score_data["news_reason"] = NEWS_REASONS.get(symbol_clean, "Varliga ait ozel finansal haber akislari anlik inceleniyor.")
