...
async def scan_once():
    ...
    for symbol in symbols:
        try:
            # Burayı da async yapıya uygun olarak time_series ile güncelledik
...
async def get_signal(symbol: str):
    ...
    if symbol in LAST:
        return LAST[symbol]
    td = TwelveData()
    # Hata veren get_prices satırını 'await td.time_series' olarak düzelttik
    data = await td.time_series(symbol)
...
async def get_chart(symbol: str):
    ...
    if data is None or data.empty:
        raise HTTPException(status_code=404, detail="Symbol not found")
    # Pandas DataFrame verisini frontend'in okuyabileceği sözlük (dict) formatına çeviriyoruz
